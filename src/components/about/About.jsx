...
export default function About({ref,width}){
    const modal=useRef()
    const text=useRef()
    const image=useRef()
    const label=useRef()
    ...
    let classes="bg-[rgba(33,38,45,0.8)] text-base sm:text-lg text-[#f0f6fc] px-4 sm:px-8 py-2 sm:py-4 rounded-lg my-9" 
        ...
        <div className="flex items-center flex-col">
            <section className="py-16 flex items-center justify-center" ref={ref} id="about">
                ...
                <div className="flex flex-col justify-center items-center max-w-5/6">
                    <span className="text-center text-4xl sm:text-5xl font-semibold mb-15 bg-gradient-to-b from-[#39d353] to-[#58a6ff] bg-clip-text text-transparent border-b-4 pb-4 border-[#58a6ff]" ref={label}>About me</span>
                    <div className="flex flex-col md:grid lg:grid-cols-2 gap-10 sm:mx-10">
                        <img src={img} alt="profile photo" className="max-h-[716px] place-self-center rounded-md" ref={image}/>
                        <div className="place-content-center" ref={text}>
